...
const GalleryCourseCard = async ({ course }: Props) => {
      ...
      <div className="border rounded-lg border-secondary">
        <div className="relative">
          ...
          >
            ...
            />
            <span className="absolute capitalize px-2 py-1 text-white rounded-md bg-black/60 w-fit bottom-2 left-2 right-2">
              {course.name}
            </span>
          </Link>
        </div>
            ...
            {course.units.map((unit, unitIndex) => {
              return (
                <Link
                  ...
                  key={unit.id}
                  className="block underline w-fit capitalize"
                >
  ...
  //   return <div>GalleryCourseCard</div>;
};
